...
flowScheduler.add(cfvgbfdaeRoutineBegin());
flowScheduler.add(cfvgbfdaeRoutineEachFrame());
flowScheduler.add(cfvgbfdaeRoutineEnd());
...
async function experimentInit() {
  ...
  // Initialize components for Routine "cfvgbfdae"
  cfvgbfdaeClock = new util.Clock();
  text_6 = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_6',
    text: 'Погоди чуть чуть...',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0], draggable: false, height: 0.05,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'LTR',
...
var cfvgbfdaeMaxDurationReached;
var cfvgbfdaeMaxDuration;
var cfvgbfdaeComponents;
function cfvgbfdaeRoutineBegin(snapshot) {
          return async function () {
            TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
            
            //--- Prepare to start Routine 'cfvgbfdae' ---
            t = 0;
            frameN = -1;
            continueRoutine = true; // until we're told otherwise
            // keep track of whether this Routine was forcibly ended
            routineForceEnded = false;
            cfvgbfdaeClock.reset(routineTimer.getTime());
            routineTimer.add(1.000000);
            cfvgbfdaeMaxDurationReached = false;
            // update component parameters for each repeat
            ...
            body: JSON.stringify({ 
            experimentID: 'dWOkHGYN2cQc', // * обновить, указав experiment ID из DATAPIPE на шаге 4.3 * 
            filename: filename,  
            data: data, 
            }), 
            ...
            psychoJS.experiment.addData('cfvgbfdae.started', globalClock.getTime());
            cfvgbfdaeMaxDuration = null
            // keep track of which components have finished
            cfvgbfdaeComponents = [];
            cfvgbfdaeComponents.push(text_6);
            
            cfvgbfdaeComponents.forEach( function(thisComponent) {
              if ('status' in thisComponent)
                thisComponent.status = PsychoJS.Status.NOT_STARTED;
               });
            return Scheduler.Event.NEXT;
...
function cfvgbfdaeRoutineEachFrame() {
          return async function () {
            //--- Loop for each frame of Routine 'cfvgbfdae' ---
            // get current time
            t = cfvgbfdaeClock.getTime();
            frameN = frameN + 1;// number of completed frames (so 0 is the first frame)
            // update/draw components on each frame
...
function cfvgbfdaeRoutineEnd(snapshot) {
          return async function () {
            //--- Ending Routine 'cfvgbfdae' ---
            cfvgbfdaeComponents.forEach( function(thisComponent) {
              if (typeof thisComponent.setAutoDraw === 'function') {
                thisComponent.setAutoDraw(false);
              }
            });
            psychoJS.experiment.addData('cfvgbfdae.stopped', globalClock.getTime());
            if (routineForceEnded) {
                routineTimer.reset();} else if (cfvgbfdaeMaxDurationReached) {
                cfvgbfdaeClock.add(cfvgbfdaeMaxDuration);
            } else {
                cfvgbfdaeClock.add(1.000000);
            }
            // Routines running outside a loop should always advance the datafile row
            if (currentLoop === psychoJS.experiment) {
              psychoJS.experiment.nextEntry(snapshot);
